      ...
      <div class="" style="margin-top:90px;">
        <div class="col-lg-8">
                    <div class="page-header">
                        <h2 id="tables">Εμφάνιση του συνόλου των οδηγων που εργάζονται όλα τα υποκαταστήματα της εταιρίας (join query)</h2>
                    </div>
    ...
    <div class="panel-body">
      <table class="table table-striped table-bordered table-list">
        <thead>
          ...
          <th ng-click="sort('first_name')">ΟΝΟΜΑ <span class="glyphicon sort-icon" ng-show="sortKey=='first_name'" ng-class="{'glyphicon-chevron-up':reverse,'glyphicon-chevron-down':!reverse}"></span></th>
          ...
          <th ng-click="sort('d_license')">Α. ΟΔΗΓΗΣΗΣ <span class="glyphicon sort-icon" ng-show="sortKey=='d_license'" ng-class="{'glyphicon-chevron-up':reverse,'glyphicon-chevron-down':!reverse}"></span></th>
          </tr>
        </thead>
        <tbody>
          <!-- Display records -->
          <tr dir-paginate="query in queries|orderBy:sortKey:reverse|filter:search|itemsPerPage:10">
          <td class="text-nowrap">{{query.store_id}}</td>
          <td>{{query.town}}</td>
          <td>{{query.first_name}}</td>
          <td>{{query.last_name}}</td>
          <td>{{query.afm}}</td>
          <td>{{query.d_license}}</td>
          </tr>
        </tbody>
          </table>
        </div>
